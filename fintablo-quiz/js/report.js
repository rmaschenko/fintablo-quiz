...
function renderFindings(a, m) {
  ...
  if (m.automationLevel < 40) {
    findings.push({ icon: '🔄', title: 'Критический уровень рутины: ' + a.manualWorkPct + '%', type: 'danger',
      body: 'Вы тратите ' + declensionHours(m.routineHours) + ' в месяц на операционные задачи. Это ' + declensionHours(m.routineFreedHours) + ', которые можно высвободить для роста.',
    ...
    findings.push({ icon: '🔄', title: 'Рутина занимает ' + a.manualWorkPct + '% времени', type: 'warning',
      body: 'Каждые 10% снижения рутины высвобождают ~' + declensionHours(Math.round(m.totalHours * 0.1)) + ' в месяц. Это время на нового клиента или повышение качества работы.',
  ...
  if (a.barriers && a.barriers.length > 0) {
    ...
    var barrierFindings = {
      'Нет стабильного потока новых клиентов': { icon: '📥', title: 'Главный барьер — отсутствие потока', type: 'warning',
        ...
        action: 'Решение: партнёрская программа помогает найти первых клиентов через входящие заявки' },
      'Не умею / не хочу активно продавать': { icon: '🎤', title: 'Барьер — нежелание продавать', type: 'warning',
        ...
        action: 'Решение: партнёрский канал позволяет получать клиентов без активных продаж — вы подтверждаете экспертизу' },
      'Много рутины — нет времени на рост': { icon: '⏰', title: 'Рутина блокирует рост', type: 'warning',
        ...
        action: 'Решение: автоматизация операционных процессов высвободит ' + declensionHours(m.routineFreedHours) + ' в месяц' }
    };
...
function getMetricRec(type, m) {
  switch (type) {
    ...
    case 'automation':
      if (m.automationLevel < 40) return 'Автоматизация рутины высвободит ~' + declensionHours(m.routineFreedHours) + '/мес. Начните с одного процесса: автоматический сбор данных или шаблонизация отчётов. Финтабло сокращает рутину в среднем на 60% у партнёров.';
    ...
    case 'source':
      if (m.sourceStability < 40) return 'Зависимость от одного канала — главный риск для стабильности дохода. Партнёрский канал помогает находить клиентов без активных продаж с вашей стороны.';
      if (m.sourceStability < 70) return 'Два канала — хороший старт. Партнёрский канал может стать дополнительным источником заявок без затрат вашего времени на продажи.';
...
function renderProfile(a, m) {
  ...
  var points = {
    ...
    'Перегруженный эксперт': ['Автоматизировать ~' + declensionHours(m.routineFreedHours) + ' рутины в месяц', 'Повысить чек на 25% у наиболее лояльных клиентов', 'Отказаться от клиентов с самой низкой отдачей на ваше время'],
...
function renderLostPotential(a, m) {
  ...
  // Always show — either rate gap or income gap or both
  var rateLoss = m.lostPotential;
  var incomeMonthly = m.incomeGap;
  var monthly = Math.max(rateLoss, incomeMonthly, 10000); // minimum 10k to always show meaningful number
  var yearly = monthly * 12;
  var daily = Math.round(monthly / 22);

  el.innerHTML =
    '<div class="lost-counter">' +
      '<div class="lost-label">' + a.name + ', при текущей модели работы вы недополучаете</div>' +
      '<div class="lost-value" id="lost-counter-value">0 ₽/год</div>' +
      '<div class="lost-monthly">' + formatMoney(monthly) + ' ₽/мес</div>' +
      '<div class="lost-sub">Это ' + formatMoney(daily) + ' ₽ каждый рабочий день — деньги, которые уже есть в вашей практике, но не доходят до вас</div>' +
    '</div>' +
  ...
  animateCounter('lost-counter-value', yearly, ' ₽/год');
}
...
function renderRoadmap(a, m) {
  ...
  if (barrierIsFlow) {
    ...
    barrierAction = '<h4>Решение вашего главного барьера: повышение чека</h4><p>Сценарий разговора: «За последние месяцы я значительно расширил работу — [перечислите конкретные улучшения]. Я планирую развивать это направление, что потребует пересмотра условий с ' + formatMoney(m.checkMid) + ' до ' + formatMoney(newCheck) + ' ₽». По опыту финансового сообщества, большинство лояльных клиентов принимают обоснованное повышение.</p>';
  } else {
    barrierAction = '<h4>Новый источник клиентов</h4><p>Добавьте партнёрский канал — он даёт предсказуемый результат с минимальными затратами вашего времени на поиск клиентов.</p>';
  }

  el.innerHTML =
    '<p style="font-size:15px;color:var(--text-secondary);margin-bottom:20px;line-height:1.6">Этот план составлен под ваши данные. На каждом этапе вы работаете вместе с Финтабло — не один.</p>' +

    '<div class="roadmap-month">' +
      '<h3>Месяц 1 — Фундамент <span style="font-size:12px;color:var(--blue);font-weight:500">совместно с менеджером Финтабло</span></h3>' +
      '<div class="roadmap-week"><h4>Неделя 1–2: Аудит экономики клиентов</h4>' +
        '<p>Ваш персональный менеджер Финтабло поможет составить таблицу: клиент × чек × часы × ставка. Определите клиентов со ставкой ниже ' + formatMoney(minRate) + ' ₽/ч. <strong>Результат:</strong> чёткое понимание, кто из клиентов приносит доход, а кто создаёт нагрузку.</p></div>' +
      '<div class="roadmap-week"><h4>Неделя 2–3: Переупаковка предложения</h4>' +
        '<p>Финтабло предоставляет готовые шаблоны КП и примеры кейсов — вам не нужно придумывать с нуля. Новый формат: помогаю [тип компании] увидеть, где бизнес теряет деньги. Целевой чек: ' + formatMoney(newCheck) + ' ₽. <strong>Результат:</strong> предложение, которое продаёт результат.</p></div>' +
      '<div class="roadmap-week"><h4>Неделя 3–4: Первое внедрение автоматизации</h4>' +
        '<p>Первое внедрение Финтабло — совместно с отделом заботы. Сейчас вы тратите ~' + declensionHours(m.routineHours) + ' на рутину. Автоматизируете один процесс у 2 клиентов. <strong>Результат:</strong> минус ' + declensionHours(Math.min(m.routineHours, Math.round(m.clients * 2.5))) + ' рутины.</p></div>' +
    '</div>' +

    '<div class="roadmap-month">' +
      '<h3>Месяц 2 — Первые результаты <span style="font-size:12px;color:var(--blue);font-weight:500">партнёрский канал Финтабло</span></h3>' +
      '<div class="roadmap-week"><h4>Новые клиенты через партнёрский канал</h4>' +
        '<p>Партнёры Финтабло получают доступ к заявкам от бизнеса, который ищет внешнего финансового директора. Скорость выхода на первого клиента зависит от вашего опыта и портфеля. <strong>Комиссия:</strong> до 50% от выручки клиента, выплата за 3 рабочих дня.</p></div>' +
    ...
    '<div class="roadmap-month">' +
      '<h3>Месяц 3 — Масштабирование <span style="font-size:12px;color:var(--blue);font-weight:500">сообщество партнёров</span></h3>' +
      '<div class="roadmap-week"><h4>Выход на траекторию ' + formatMoneyShort(m.targetIncome) + '/мес</h4>' +
        '<p>' + (m.newClientsNeeded <= 2
          ? 'Достаточно ' + m.newClientsNeeded + ' новых клиентов через Финтабло + повышение чека у текущих.'
          : 'Нужно ' + m.newClientsNeeded + ' новых клиентов — реалистично за 3–5 месяцев через партнёрский канал.') +
        ' Вы уже в профессиональном сообществе партнёров Финтабло — разбор сложных кейсов, обмен опытом, поддержка коллег. <strong>Результат:</strong> система привлечения работает, рутина снижена, вы фокусируетесь на экспертизе.</p></div>' +
    '</div>' +

    '<div class="report-insight">' +
      '<strong>Вы идёте этот путь не один.</strong> На каждом этапе плана Финтабло обеспечивает конкретную поддержку:<br><br>' +
      '<strong>Месяц 1</strong> — готовые шаблоны для переупаковки предложения, персональный менеджер помогает с первым внедрением.<br>' +
      '<strong>Месяц 2</strong> — доступ к заявкам от бизнеса через партнёрский канал. Вам не нужно искать клиентов самостоятельно. Автоматизация сокращает рутину с ' + a.manualWorkPct + '% до ~20%.<br>' +
...
function renderFinTablo(a, m) {
  ...
  // Vibe-coding comparison removed — covered by Scenario Г in renderScenarios()
  ...
  el.innerHTML =
    ...
    '<div class="benefit-card"><div class="benefit-icon">📉</div><h4>Избавьтесь от рутины в финансовом учёте</h4>' +
      '<p>Сейчас: ' + a.manualWorkPct + '% времени на рутину → С Финтабло: ~20%<br>Высвобождается: ' + declensionHours(m.routineFreedHours) + ' в месяц<br>Это время на ' + freedClients + ' дополнительных клиентов или на повышение качества работы с текущими</p></div>' +
    ...
    '<div class="benefit-card"><div class="benefit-icon">📥</div><h4>Входящий поток заявок</h4>' +
      '<p>Партнёры получают доступ к заявкам от бизнеса, который ищет внешнего финансового директора. Канал помогает находить клиентов без активных продаж с вашей стороны.</p></div>' +

    '<div class="benefit-card"><div class="benefit-icon">🤝</div><h4>Экспертное сообщество</h4>' +
      '<p>Закрытое профессиональное сообщество партнёров Финтабло. Разбор сложных кейсов, менторство при первом внедрении, профессиональная поддержка от коллег и экспертов.</p></div>' +
    ...
    '<div style="margin:20px 0">' +
      ...
      '<div style="background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);padding:16px;margin-bottom:10px">' +
        ...
        '<div style="font-size:13px;color:var(--accent-green);font-weight:500;margin-top:6px">Клиенты через партнёрскую программу</div>' +
      '</div>' +
