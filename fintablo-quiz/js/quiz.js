...
var INSIGHTS = {
  ...
  clients: function(n) {
    if (n === 0) return 'Нулевая точка — но не нулевой потенциал. Правильный канал привлечения клиентов — ключевое решение на старте. Финансисты с опытом от 3 лет выходят на первого клиента быстрее всего через партнёрские программы.';
  ...
  sources: function(sources) {
    if (sources.some(function(s) { return s.indexOf('Пока клиентов') >= 0; })) return 'Честная точка старта. Первый клиент через рекомендации появляется в среднем через 2–4 месяца. Партнёрский канал с готовым потоком заявок — значительно быстрее. Выбор источника определяет скорость старта.';
...
function renderCapturePreview() {
  ...
  if (lossEl) {
    ...
    lossEl.innerHTML =
      ...
      '<div class="cl-monthly">' + formatMoney(monthly) + ' ₽/мес</div>' +
...
/* ============ HELPERS ============ */
/* declension() and declensionHours() are in calculator.js (shared) */
